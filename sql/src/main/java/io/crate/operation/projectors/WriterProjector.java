...
class WriterProjector extends AbstractProjector {
     ...
     */
    WriterProjector(ExecutorService executorService,
                    String uriString,
        ...
        try {
            ...
            throw new ValidationException(String.format(Locale.ENGLISH, "Invalid uri '%s'", uriString), e);
        }
        if (uri.getScheme() == null || uri.getScheme().equals("file")) {
            this.output = new OutputFile(uri, compressionType);
        } else if (uri.getScheme().equalsIgnoreCase("s3")) {
            this.output = new OutputS3(executorService, uri, compressionType);
        } else {
    ...
    private static class DocWriter implements RowWriter {
        ...
        DocWriter(OutputStream outputStream,
                  Set<CollectExpression<Row, ?>> collectExpressions,
    ...
    private static class RawRowWriter implements RowWriter {
    ...
    private static class ColumnRowObjectWriter extends ColumnRowWriter {

        private final List<String> outputNames;

        ColumnRowObjectWriter(OutputStream outputStream,
                              Set<CollectExpression<Row, ?>> collectExpressions,
                              List<Input<?>> inputs,
