...
import io.crate.planner.fetch.NodeRewritingVisitor;
...
public class Planner extends AnalyzedStatementVisitor<Planner.Context, Plan> {
    ...
    public static class Context {
        ...
        private Routing rebuildRouting(Routing routing) {
            NodeRewritingVisitor rebuilder = new NodeRewritingVisitor(shardNodes);
            routing.walkLocations(rebuilder);
            return new Routing(rebuilder.build());
        }
        ...
        public Routing allocateRouting(TableInfo tableInfo, WhereClause where, @Nullable String preference) {
            ...
            if (existingRoutings.isEmpty()) {
                ...
                if (!allocateRoutingNodes(routing.locations())) {
                    routing = rebuildRouting(routing);
                }
            }
